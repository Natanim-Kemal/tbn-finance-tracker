...
const userController = {
  ...
  getAccountDetails: async (req, res) => {
    ...
    try {
      ...
      res
        .status(accountDetails ? 200 : 404)
        .json(accountDetails || { message: "User not found" });
    } catch (error) {
  ...
  updateProfile: async (req, res) => {
    ...
    try {
      ...
      res.status(result.success ? 200 : 500).json(result.updatedProfile);
    } catch (error) {
  ...
  getFinancialReport: async (req, res) => {
    ...
    try {
      ...
      res
        .status(financialReport ? 200 : 500)
        .json(financialReport || { message: "Server Error" });
    } catch (error) {
